...
* npm init : in creates package json file to store verison of packge
* Bundlers:  to make production ready code need to use bundler where it combines all code,
       By Default 'React Build app' uses webpack bundler.
       for pracel using parcel bundler it is zero config bundler so.
* Dependencies: dev and normal (production)
    npm install : ininstall node modeles and package-jsonlock
    npm insatll -D parcel:  after this parcel version get added in package config file.
    npm install react: using cdn in html script is good way to get react. >> after this react version get added in package config file.
    npm install reactdom:
    
    after npm install react and reactdom can use ,import react top of js file.
    some people use js file as jsx , js, any extension works.
...
* config steps:
  ...
  3. npm i react
  4. npm i react-dom
  5. npx build index.html   to sttart locally
* .git-ignore:  node-modules,dist, .parcel-cache  no need to push it can download with npm with version prasent in packge file.
